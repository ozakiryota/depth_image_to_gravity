...
class TrainModel:
    def __init__(self,
        ...
        self.str_hyperparameter  = self.getStrHyperparameter(method_name, resize, mean_element, std_element, optimizer_name, lr_colorcnn, lr_depthcnn, lr_fc, batch_size)
    ...
    def getNetwork(self, resize):
        net = network_mod.Network(resize, use_pretrained=True)
        print(net)
        net.to(self.device)
        return net

    def getOptimizer(self, optimizer_name, lr_colorcnn, lr_depthcnn, lr_fc):
        ## param
        list_colorcnn_param_value, list_depthcnn_param_value, list_fc_param_value = self.net.getParamValueList()
        ## optimizer
        if optimizer_name == "SGD":
            optimizer = optim.SGD([
                {"params": list_colorcnn_param_value, "lr": lr_colorcnn},
                {"params": list_depthcnn_param_value, "lr": lr_depthcnn},
    ...
    def getStrHyperparameter(self, method_name, resize, mean_element, std_element, optimizer_name, lr_colorcnn, lr_depthcnn, lr_fc, batch_size):
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for phase in ["train", "val"]:
                ...
                ## data load
                epoch_loss = 0.0
                for inputs_color, inputs_depth, labels in tqdm(self.dataloaders_dict[phase]):
